...
v1.5.3 adds a taskbar-frame XAML visual-tree scan. Earlier logs showed that:

- old task-list pinned groups exist on both task lists;
- Taskbar.View symbols are resolved;
- TaskListButton::DisplayName / UpdateVisualStates don't fire after the hook.

This version also hooks:

- `TaskbarFrame::OnTaskbarLayoutChildBoundsChanged`

and scans the visual tree for `Taskbar.TaskListButton` elements. Search logs for:

- `TaskbarFrame layout changed`
- `VisualTree TaskListButton`
- `VisualTree scan complete`
...
static void MakeElementVisible(FrameworkElement element, PCWSTR reason, PCWSTR name){
    ...
    try{
        ...
        static int v=0;
        if(v<500){
            DebugLog(L"Made XAML element visible: reason=%s, class=%s, name=%s, element=%p",
                reason,
                winrt::get_class_name(element).c_str(),
                name?name:element.Name().c_str(),
                winrt::get_abi(element));
            v++;
        }
    }catch(...){
        static int e=0;if(e<30){DebugLog(L"MakeElementVisible exception: reason=%s",reason);e++;}
    }
}
...
static void ScanVisualTreeForTaskListButtons(winrt::Windows::UI::Xaml::DependencyObject root,int depth,int* foundCount){
    ...
    int count=0;
    try{count=Media::VisualTreeHelper::GetChildrenCount(root);}catch(...){return;}
    for(int i=0;i<count;i++){
        ...
        if(fe){
            auto cls=winrt::get_class_name(fe);
            auto name=fe.Name();
            if(cls==L"Taskbar.TaskListButton" || name==L"TaskListButton"){
                (*foundCount)++;
                static int n=0;
                if(n<500){
                    DebugLog(L"VisualTree TaskListButton: depth=%d index=%d element=%p class=%s name=%s visibility=%d opacity=%.2f actualWidth=%.1f actualHeight=%.1f",
                        depth,i,winrt::get_abi(fe),cls.c_str(),name.c_str(),(int)fe.Visibility(),fe.Opacity(),fe.ActualWidth(),fe.ActualHeight());
                    n++;
                }
                MakeElementVisible(fe,L"VisualTree",name.c_str());
            }
        }
...
static int WINAPI CTaskListWnd_IsOnPrimaryTaskband_Hook(PVOID pThis){
    static int n=0;
    if(g_settings.showPinnedOnAllTaskbars){if(n<100){DebugLog(L"IsOnPrimaryTaskband called, returning TRUE, this=%p",pThis);n++;}return TRUE;}
...
static void WINAPI CTaskListWnd_HandleTaskGroupPinned_Hook(PVOID pThis,PVOID taskGroup){
    static int n=0;if(n<100){DebugLog(L"HandleTaskGroupPinned called, this=%p, taskGroup=%p",pThis,taskGroup);n++;}
...
static PVOID WINAPI CTaskListWnd__CreateTBGroup_Hook(PVOID pThis,PVOID taskGroup,int index){
    static int n=0;if(n<160){DebugLog(L"_CreateTBGroup called, this=%p, taskGroup=%p, index=%d",pThis,taskGroup,index);n++;}
...
static LONG_PTR WINAPI CTaskListWnd__TaskCreated_Hook(PVOID pThis,PVOID taskGroup,PVOID taskItem,int param3){
    static int n=0;if(n<200){DebugLog(L"_TaskCreated called, this=%p, taskGroup=%p, taskItem=%p, param3=%d",pThis,taskGroup,taskItem,param3);n++;}
    ...
    if(g_settings.showPinnedOnAllTaskbars&&!taskItem&&taskGroup&&CTaskListWnd__GetTBGroupFromGroup_Original&&CTaskListWnd__CreateTBGroup_Original){
        int foundIndex=-1;PVOID existing=CTaskListWnd__GetTBGroupFromGroup_Original(pThis,taskGroup,&foundIndex);
        static int c=0;if(c<200){DebugLog(L"Pinned check: this=%p, taskGroup=%p, existing=%p, foundIndex=%d, ret=%lld",pThis,taskGroup,existing,foundIndex,ret);c++;}
        if(!existing){PVOID created=CTaskListWnd__CreateTBGroup_Original(pThis,taskGroup,kDaLast);DebugLog(L"Created missing pinned TBGroup: this=%p, taskGroup=%p, created=%p",pThis,taskGroup,created);}
    }
    return ret;
}
...
static void WINAPI TaskListButton_DisplayName_Hook(void* pThis,winrt::hstring name){
    ...
    static int n=0;if(n<300){DebugLog(L"TaskListButton DisplayName: this=%p, name=%s",pThis,name.c_str());n++;}
...
static void WINAPI TaskListButton_UpdateVisualStates_Hook(void* pThis){
    ...
    static int n=0;if(n<500){DebugLog(L"TaskListButton UpdateVisualStates: this=%p",pThis);n++;}
    TryMakeTaskListButtonVisible(pThis,L"UpdateVisualStates",nullptr);
}

static void WINAPI TaskbarFrame_OnTaskbarLayoutChildBoundsChanged_Hook(void* pThis){
    TaskbarFrame_OnTaskbarLayoutChildBoundsChanged_Original(pThis);
    static int n=0;if(n<120){DebugLog(L"TaskbarFrame layout changed: this=%p",pThis);n++;}
    if(!g_settings.showPinnedOnAllTaskbars){return;}
    try{
        void* unkPtr=(void**)pThis+3;
        winrt::Windows::Foundation::IUnknown unk;
        winrt::copy_from_abi(unk,unkPtr);
        FrameworkElement frame=unk.try_as<FrameworkElement>();
        if(!frame){DebugLog(L"TaskbarFrame layout: no FrameworkElement");return;}
        int found=0;
        ScanVisualTreeForTaskListButtons(frame,0,&found);
        static int s=0;if(s<120){DebugLog(L"VisualTree scan complete: frame=%p foundTaskListButtons=%d",winrt::get_abi(frame),found);s++;}
    }catch(...){
        static int e=0;if(e<30){DebugLog(L"TaskbarFrame visual-tree scan exception: this=%p",pThis);e++;}
    }
}
...
static BOOL CALLBACK RefreshTaskbarEnumProc(HWND hWnd, LPARAM){
    ...
    WCHAR cls[64]{};if(!GetClassNameW(hWnd,cls,ARRAYSIZE(cls))){return TRUE;}
    if(_wcsicmp(cls,L"Shell_TrayWnd")==0||_wcsicmp(cls,L"Shell_SecondaryTrayWnd")==0){DebugLog(L"RefreshTaskbar: sending WM_SETTINGCHANGE to %s hwnd=%p",cls,hWnd);SendMessageW(hWnd,WM_SETTINGCHANGE,0,0);}
    return TRUE;
}
...
static bool GetTaskbarViewDllPath(WCHAR path[MAX_PATH]){
    WCHAR winDir[MAX_PATH];if(!GetWindowsDirectoryW(winDir,ARRAYSIZE(winDir)))return false;
    wcscpy_s(path,MAX_PATH,winDir);wcscat_s(path,MAX_PATH,LR"(\SystemApps\MicrosoftWindows.Client.Core_cw5n1h2txyewy\Taskbar.View.dll)");if(GetFileAttributesW(path)!=INVALID_FILE_ATTRIBUTES)return true;
    wcscpy_s(path,MAX_PATH,winDir);wcscat_s(path,MAX_PATH,LR"(\SystemApps\MicrosoftWindows.Client.CBS_cw5n1h2txyewy\ExplorerExtensions.dll)");if(GetFileAttributesW(path)!=INVALID_FILE_ATTRIBUTES)return true;
    return false;
}

static bool HookTaskbarViewSymbols(){
    if(!GetTaskbarViewDllPath(g_taskbarViewDllPath)){DebugLog(L"Taskbar view dll path not found");return false;}
    HMODULE module=LoadLibraryExW(g_taskbarViewDllPath,nullptr,LOAD_WITH_ALTERED_SEARCH_PATH);if(!module){DebugLog(L"Load taskbar view dll failed: %s",g_taskbarViewDllPath);return false;}
    WindhawkUtils::SYMBOL_HOOK hooks[]={
        ...
        {{LR"(private: void __cdecl winrt::Taskbar::implementation::TaskbarFrame::OnTaskbarLayoutChildBoundsChanged(void))",LR"(private: void __cdecl winrt::Taskbar::implementation::TaskbarFrame::OnTaskbarLayoutChildBoundsChanged(void) __ptr64)"},&TaskbarFrame_OnTaskbarLayoutChildBoundsChanged_Original,TaskbarFrame_OnTaskbarLayoutChildBoundsChanged_Hook,true},
    };
    if(!HookSymbols(module,hooks,ARRAYSIZE(hooks))){DebugLog(L"Hook Taskbar.View symbols failed");return false;}
    g_taskbarViewHooked=true;DebugLog(L"Hooked Taskbar.View symbols: module=%p path=%s DisplayName=%p UpdateVisualStates=%p FrameLayout=%p",module,g_taskbarViewDllPath,TaskListButton_DisplayName_Original,TaskListButton_UpdateVisualStates_Original,TaskbarFrame_OnTaskbarLayoutChildBoundsChanged_Original);return true;
}

static HMODULE GetTaskbarModuleForSymbols(){
    HMODULE module=GetModuleHandleW(L"taskbar.dll");if(module){DebugLog(L"taskbar.dll already loaded: %p",module);return module;}
    module=LoadLibraryExW(L"taskbar.dll",nullptr,LOAD_LIBRARY_SEARCH_SYSTEM32);if(module){DebugLog(L"taskbar.dll loaded by mod: %p",module);return module;}
    HMODULE explorerModule=GetModuleHandleW(nullptr);DebugLog(L"taskbar.dll not found, using explorer.exe: %p",explorerModule);return explorerModule;
}

static bool HookTaskbarSymbols(){
    HMODULE module=GetTaskbarModuleForSymbols();if(!module){DebugLog(L"No taskbar module");return false;}
