...
- showPinnedOnAllTaskbars: false
  ...
  $description: Experimental. Tries diagnostics and visibility fixes for pinned taskbar buttons on secondary taskbars. Restart explorer.exe after changing this setting.
- keepEnforced: true
...
static bool GetTaskbarViewDllPath(WCHAR path[MAX_PATH]){
    ...
    wcscpy_s(path,MAX_PATH,winDir);wcscat_s(path,MAX_PATH,LR"(\SystemApps\MicrosoftWindows.Client.Core_cw5n1h2txyewy\Taskbar.View.dll)");
    if(GetFileAttributesW(path)!=INVALID_FILE_ATTRIBUTES)return true;
    wcscpy_s(path,MAX_PATH,winDir);wcscat_s(path,MAX_PATH,LR"(\SystemApps\MicrosoftWindows.Client.CBS_cw5n1h2txyewy\ExplorerExtensions.dll)");
    if(GetFileAttributesW(path)!=INVALID_FILE_ATTRIBUTES)return true;
    return false;
}
...
static bool HookTaskbarViewSymbols(){
    ...
    HMODULE module=LoadLibraryExW(g_taskbarViewDllPath,nullptr,LOAD_WITH_ALTERED_SEARCH_PATH);
...
static LSTATUS ReturnForcedDword(DWORD forcedValue,DWORD requestedFlags,LPDWORD typeOut,PVOID dataOut,LPDWORD dataSizeInOut){
    constexpr DWORD RRF_RT_ANY_KNOWN=0x0000ffff;constexpr DWORD RRF_RT_REG_DWORD_CONST=0x00000010;
    DWORD typeFilter=requestedFlags&RRF_RT_ANY_KNOWN;if(typeFilter&&!(typeFilter&RRF_RT_REG_DWORD_CONST))return ERROR_FILE_NOT_FOUND;
    if(typeOut)*typeOut=REG_DWORD;if(dataSizeInOut){if(!dataOut){*dataSizeInOut=sizeof(DWORD);return ERROR_SUCCESS;}if(*dataSizeInOut<sizeof(DWORD)){*dataSizeInOut=sizeof(DWORD);return ERROR_MORE_DATA;}*dataSizeInOut=sizeof(DWORD);}if(dataOut)*reinterpret_cast<DWORD*>(dataOut)=forcedValue;return ERROR_SUCCESS;
}
...
BOOL Wh_ModInit(){
    ...
    LoadSettings();
    ReadDwordFromAdvancedKey(kValueTaskbarEnabled,&g_backupEnabled);ReadDwordFromAdvancedKey(kValueTaskbarMode,&g_backupMode);
    ...
    HMODULE advapi32=GetModuleHandleW(L"advapi32.dll");if(!advapi32)advapi32=LoadLibraryW(L"advapi32.dll");if(!advapi32){DebugLog(L"No advapi32");return FALSE;}
    void* regGetValue=reinterpret_cast<void*>(GetProcAddress(advapi32,"RegGetValueW"));void* regQueryValueEx=reinterpret_cast<void*>(GetProcAddress(advapi32,"RegQueryValueExW"));
